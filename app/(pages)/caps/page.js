...
const getCaps = async () => {
  const caps = await fetch(`${process.env.HOST}/api/getproducts`, {
    cache: "no-store",
  });
  const json = await caps.json();

  const res = Object.values(json).filter((e) => {
    return e.category === "cap";
  });
      ...
      <section className="text-gray-600 dark:text-dark-primaryText body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap md:flex-nowrap -m-4">
            {caps.length === 0 ? (
              <p>All Caps Sold Out. New Stock Coming Soon! Stay Tuned...</p>
            ) : (
              caps.map((item) => {
                return (
                  <Link
